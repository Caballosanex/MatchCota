...
| 05-05-03 | 05 | 3 | ONBD-06, FRONT-05 | T-05-16 | Frontend production API base URL normalizes to `/api/v1` and registration no longer targets root `/tenants/` path | lint/build/deploy | `cd frontend && npm run lint && npm run build && AWS_PROFILE=matchcota AWS_REGION=us-east-1 FRONTEND_HOST=18.210.253.176 DEPLOY_TRANSPORT=ssm bash infrastructure/scripts/deploy-frontend.sh` | ✅ | ✅ green |
